...
RUN ./node_modules/.bin/prisma generate \
  ...
  && test -n "$(find node_modules -name 'schema-engine-debian-openssl-3*' | head -n 1)"
...
# openssl(1) lets Prisma detect debian-openssl-3.0.x at migrate time on a read-only root FS.
